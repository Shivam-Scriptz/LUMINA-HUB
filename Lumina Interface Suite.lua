...
local LuminaLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Lumina
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across Lumina
...
local Lumina = game:GetObjects("rbxassetid://10804731440")[1]

Lumina.Enabled = false


if gethui then
	Lumina.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(Lumina)
	Lumina.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Lumina.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Lumina.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Lumina.Name and Interface ~= Lumina then
			Interface.Enabled = false
			Interface.Name = "Lumina-Old"
		end
	end
else
...
local Main = Lumina.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Lumina.DisplayOrder = 100
...
local Notifications = Lumina.Notifications

local SelectedTheme = LuminaLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = LuminaLibrary.Theme[ThemeName]
	for _, obj in ipairs(Lumina:GetDescendants()) do
	...
	Lumina.Main.BackgroundColor3 = SelectedTheme.Background
	Lumina.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Lumina.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Lumina.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Lumina.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Lumina.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Lumina.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if LuminaLibrary.Flags[FlagName] then
			spawn(function() 
				if LuminaLibrary.Flags[FlagName].Type == "ColorPicker" then
					LuminaLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if LuminaLibrary.Flags[FlagName].CurrentValue or LuminaLibrary.Flags[FlagName].CurrentKeybind or LuminaLibrary.Flags[FlagName].CurrentOption or LuminaLibrary.Flags[FlagName].Color ~= FlagValue then LuminaLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			LuminaLibrary:Notify({Title = "Flag Error", Content = "Lumina was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(LuminaLibrary.Flags) do
...
function LuminaLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= LuminaLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Lumina | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	LuminaLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function LuminaLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Lumina Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Lumina Interface Suite" then
		LoadingFrame.Version.Text = "Lumina UI"
	end
	...
	if Settings.KeySystem then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Lumina | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Lumina | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					LuminaLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Lumina | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Lumina | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					LuminaLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Lumina | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					LuminaLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Lumina | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					LuminaLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Lumina | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					LuminaLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				...
				LuminaLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, LuminaLibrary.LoadConfiguration, LuminaLibrary)
